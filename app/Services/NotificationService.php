...
{
    /**
     * Build notifications list and unread count based on request context.
     */
    public function getNotificationsForRequest(Request $request): array
    {
        $user = $request->user();
        $now = Carbon::now();
        $notifications = [];
        ...
        if ($isClientContext) {
            $notifications = $this->getClientNotifications($user, $request, $now);
        } else {
            $notifications = $this->getAdminNotifications($now);
        }
    ...
    /**
     * Generate notifications for Client Portal context.
     */
    protected function getClientNotifications(?User $user, Request $request, Carbon $now): array
    {
        $notifications = [];
        $client = null;
    ...
    /**
     * Generate notifications for Admin / Studio Operation context.
     */
    protected function getAdminNotifications(Carbon $now): array
    {
        $notifications = [];
