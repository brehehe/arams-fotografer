...
export default function ProjectsEdit({
    ...
    const [projectDeadline, setProjectDeadline] = useState<string>(initialDueDate);
    ...
    const [categoryData, setCategoryData] = useState<AnyCategorySpecificData>(() => {
        const existing = (project.category_data && typeof project.category_data === 'object') ? project.category_data : {};
        const cl = project.client || clients.find((c) => String(c.id) === String(project.client_id));
        return {
            bride_name: existing.bride_name || cl?.bride_name || '',
            groom_name: existing.groom_name || cl?.groom_name || '',
            partner_1: existing.partner_1 || cl?.bride_name || '',
            ...
            pic_phone: existing.pic_phone || cl?.phone || '',
            pic_email: existing.pic_email || cl?.email || '',
            children: existing.children && Array.isArray(existing.children)
                ? existing.children
    ...
    const initialTeamAssignments: TeamAssignmentItem[] = useMemo(() => {
        const catAssignments = (project.category_data as any)?.team_assignments;
        if (Array.isArray(catAssignments) && catAssignments.length > 0) {
            return catAssignments.map((item: any, idx: number) => ({
                id: item.id || `team-init-${idx}`,
                type: item.type || 'Photografer',
                name: item.name || '',
            }));
        }

        const list: TeamAssignmentItem[] = [];
        const photo = project.photographer_name || project.photographer?.name || '';
        const editor = project.editor_name || project.editor?.name || '';

        if (photo) {
            list.push({ id: 'team-photo-1', type: 'Photografer', name: photo });
        }
        if (editor) {
            list.push({ id: 'team-editor-1', type: 'Editor Foto', name: editor });
        }
        if (list.length === 0) {
            list.push({ id: 'team-default-1', type: 'Photografer', name: '' });
            list.push({ id: 'team-default-2', type: 'Editor Foto', name: '' });
        }
        return list;
    }, [project.category_data, project.photographer_name, project.editor_name, project.photographer, project.editor]);
    ...
    const [teamAssignments, setTeamAssignments] = useState<TeamAssignmentItem[]>(initialTeamAssignments);
    ...
    // Payment Installments Init
    const initialPaymentInstallments: PaymentInstallmentItem[] = useMemo(() => {
        const catInstallments = (project.category_data as any)?.payment_installments;
        if (Array.isArray(catInstallments) && catInstallments.length > 0) {
            return catInstallments.map((inst: any, idx: number) => ({
                id: inst.id || `inst-${idx}`,
                name: inst.name || `Invoice ${idx + 1}`,
                type: inst.type || (idx === 0 ? 'dp' : 'termin'),
        ...
        if (project.invoices && Array.isArray(project.invoices) && project.invoices.length > 0) {
            return project.invoices.map((inv: any, idx: number) => ({
                id: inv.id || `inv-${idx}`,
                name: inv.notes?.replace(/ untuk .*$/, '') || `Invoice ${idx + 1}`,
                type: (idx === 0 ? 'dp' : (idx === project.invoices.length - 1 ? 'pelunasan' : 'termin')) as any,
        ...
        const totalP = Number(project.total_amount) || 0;
        const dpVal = Number(project.paid_amount) || Math.round(totalP * 0.3);
        const remVal = Math.max(0, totalP - dpVal);
        const fallbackDate = initialDueDate || new Date().toISOString().split('T')[0];
        const fallbackEventDate = initialEventDate || fallbackDate;
        return [
            {
                id: 'inst-1',
                name: 'Invoice 1 (DP)',
            ...
            },
            {
                ...
                type: 'pelunasan',
                amount: remVal,
                due_date: fallbackEventDate,
            },
        ];
    }, [project.category_data, project.invoices, project.total_amount, project.paid_amount, initialDueDate, initialEventDate]);

    const [paymentInstallments, setPaymentInstallments] = useState<PaymentInstallmentItem[]>(initialPaymentInstallments);
        ...
        const payload = {
            ...
            client_overrides: {
                ...
                ...(activeCategoryKey === 'prewedding' ? {
                    ...
                    groom_name: categoryData.partner_2 || null,
                } : {}),
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 min-w-0">
                            ...
                            <div className="space-y-1 min-w-0">
                                ...
                                <div className="h-[42px] px-3 bg-slate-100/80 border border-slate-200 rounded-xl text-xs font-mono font-semibold text-slate-700 flex items-center justify-between">
                                    <span>{project.project_number || 'PRJ-AUTO'}</span>
                    ...
                    <div className="bg-white p-5 sm:p-6 rounded-xl border border-slate-200/80 shadow-xs space-y-4">
                        <div className="border-b border-slate-100 pb-3 flex items-center justify-between">
                            <div className="flex items-center gap-2.5">
                                ...
                                <div>
                                    <h3 className="font-bold text-base text-slate-900">3. Paket Layanan &amp; Deliverables</h3>
                                    <p className="text-[11px] text-slate-400">Pilih paket dokumentasi dan deliverables project</p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="bg-white p-5 sm:p-6 rounded-xl border border-slate-200/80 shadow-xs space-y-4">
                        <div className="border-b border-slate-100 pb-3 flex items-center justify-between">
                            <div className="flex items-center gap-2.5">
                                ...
                                <div>
                                    <h3 className="font-bold text-base text-slate-900">4. Catatan &amp; Referensi</h3>
                                    <p className="text-[11px] text-slate-400">Catatan khusus, sumber rujukan, dan catatan tim</p>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            ...
                            <div className="space-y-1 sm:col-span-2 pt-1 border-t border-slate-100 min-w-0">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-1 gap-6 items-start text-slate-900">
                        ...
                        <div className="lg:col-span-6 bg-white p-5 sm:p-6 rounded-xl border border-slate-200/80 shadow-xs space-y-4">
                            ...
                            <ProjectTeamAssignmentSection
                                ...
                                teamSuggestions={team_members.length > 0 ? team_members : supervisors}
